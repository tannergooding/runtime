    ...
    internal readonly struct TensorShape : IEquatable<TensorShape>
    {
        ...
        private TensorShape(nint linearLength, scoped ReadOnlySpan<nint> lengths, scoped ReadOnlySpan<nint> strides, TensorFlags flags)
        {
            ...
            else
            {
                ...
                MemoryExtensions.Sort(destinationStrides);
                ...
                for (int n = 0; n < rank; n++)
                {
                    ...
                    nint orderedStride = destinationStrides[i];

                    if (stride != orderedStride)
                    {
                        // We can't be dense if we aren't ordered
                        isDense = false;
                        destinationStrides[i] = stride;
                    }
                    ...
                    if (orderedStride != 0)
                    {
                        if (orderedStride < minimumNonZeroStride)
                        {
                        ...
                        else if (orderedStride != minimumNonZeroStride)
                        {
                            isDense = false;
                        }
                        ...
                        minimumNonZeroStride = checked(length * orderedStride);
                        maximumLinearIndex = checked(maximumLinearIndex + (minimumNonZeroStride - orderedStride));
                    }
                    else
                    {
                        flags |= TensorFlags.IsBroadcast;
