...
namespace System.Numerics.Tensors
{
    ...
    internal readonly struct TensorShape : IEquatable<TensorShape>
    {
        ...
        public void Invoke<T, TOperation>(ref T destination)
            where TOperation : IOperation<T>
        {
            int rank = Rank;

            nint[]? sourceIndexesArray = null;
            InlineBuffer<nint> sourceIndexesBuffer;
            scoped Span<nint> sourceIndexes;

            if (rank > MaxInlineRank)
            {
                sourceIndexesArray = ArrayPool<nint>.Shared.Rent(rank);
                sourceIndexes = sourceIndexesArray.AsSpan(0, rank);
            }
            else
            {
                Unsafe.SkipInit(out sourceIndexesBuffer);
                sourceIndexes = ((Span<nint>)sourceIndexesBuffer)[..rank];
            }

            nint sourceLinearOffset = 0;
            sourceIndexes[^1] = -1;

            for (nint i = 0; i < FlattenedLength; i++)
            {
                sourceLinearOffset = AdjustToNextIndex(sourceLinearOffset, sourceIndexes);
                TOperation.Invoke(ref Unsafe.Add(ref destination, sourceLinearOffset));
            }

            if (sourceIndexesArray is not null)
            {
                ArrayPool<nint>.Shared.Return(sourceIndexesArray);
            }
        }

        public void Invoke<T, TOperation>(ref T destination, T value)
            where TOperation : IOperation_Scalar<T>
        {
            int rank = Rank;
            ...
            {
                sourceLinearOffset = AdjustToNextIndex(sourceLinearOffset, sourceIndexes);
                TOperation.Invoke(ref Unsafe.Add(ref destination, sourceLinearOffset), value);
            }
        ...
        public void Invoke<T, TOperation>(in TensorSpan<T> destination, ref T source)
            where TOperation : IOperation_Span<T>
        {
            int rank = Rank;

            if (destination.Rank != rank)
            {
                ThrowHelper.ThrowArgument_SpanRankMismatch();
            }
            ...
            nint[]? destinationIndexesArray = null;
            InlineBuffer<nint> destinationIndexesBuffer;
            scoped Span<nint> destinationIndexes;
            ...
            if (rank > MaxInlineRank)
            {
                ...
                destinationIndexesArray = ArrayPool<nint>.Shared.Rent(rank);
                destinationIndexes = sourceIndexesArray.AsSpan(0, rank);
            }
            else
            {
                Unsafe.SkipInit(out sourceIndexesBuffer);
                ...
                Unsafe.SkipInit(out destinationIndexesBuffer);
                destinationIndexes = ((Span<nint>)destinationIndexesBuffer)[..rank];
            }

            nint sourceLinearOffset = 0;
            sourceIndexes[^1] = -1;

            nint destinationLinearOffset = 0;
            destinationIndexes[^1] = -1;

            for (nint i = 0; i < FlattenedLength; i++)
            {
                sourceLinearOffset = AdjustToNextIndex(sourceLinearOffset, sourceIndexes);
                destinationLinearOffset = AdjustToNextIndex(destinationLinearOffset, destinationIndexes);

                TOperation.Invoke(ref Unsafe.Add(ref destination._reference, destinationLinearOffset), in Unsafe.Add(ref source, sourceLinearOffset));
            }
        ...
        public interface IOperation_Scalar<T>
        {
            static abstract void Invoke(ref T destination, T value);
            static abstract void Invoke(Span<T> destination, T value);
        }

        public interface IOperation_Span<T>
        {
            static abstract void Invoke(ref T destination, in T value);
            static abstract void Invoke(Span<T> destination, ReadOnlySpan<T> value);
        }
        ...
        public readonly struct ClearOperation<T>
            : IOperation<T>
        {
        ...
        public readonly struct CopyToOperation<T>
            : IOperation_Span<T>
        {
            public static void Invoke(ref T destination, in T source)
            {
                destination = source;
            }

            public static void Invoke(Span<T> destination, ReadOnlySpan<T> source)
            {
                source.CopyTo(destination);
            }
        }

        public readonly struct FillOperation<T>
            : IOperation_Scalar<T>
        {
