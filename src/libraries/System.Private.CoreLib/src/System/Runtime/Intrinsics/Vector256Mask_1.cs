...
[DebuggerTypeProxy(typeof(Vector256MaskDebugView<>))]
[StructLayout(LayoutKind.Sequential, Size = Vector256Mask.Size)]
public readonly unsafe struct Vector256Mask<T> : IEquatable<Vector256Mask<T>>
    where T : struct
{
    // This field allows the debug view to work https://github.com/dotnet/runtime/issues/9495)
    internal readonly uint _value;

    /// <summary>Gets a new <see cref="Vector256Mask{T}" /> that includes all elements of a vector.</summary>
    /// <exception cref="NotSupportedException">The type of the vector mask (<typeparamref name="T" />) is not supported.</exception>
    public static Vector256Mask<T> AllBitsSet
    {
        [Intrinsic]
        ...
        get
        {
            ...
            return Unsafe.BitCast<uint, Vector256Mask<T>>(uint.MaxValue);
        }
    }

    /// <summary>Gets the number of <typeparamref name="T" /> that are in a <see cref="Vector256Mask{T}" />.</summary>
    /// <exception cref="NotSupportedException">The type of the vector mask (<typeparamref name="T" />) is not supported.</exception>
...
#pragma warning disable 8500 // sizeof of managed types
            return Vector256Mask.Size / sizeof(T);
#pragma warning restore 8500
    ...
    /// <summary>Gets a new <see cref="Vector256Mask{T}" /> that includes no elements of a vector.</summary>
    /// <exception cref="NotSupportedException">The type of the vector mask (<typeparamref name="T" />) is not supported.</exception>
    public static Vector256Mask<T> Zero
    {
        [Intrinsic]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        get
    ...
    public static bool operator ==(Vector256Mask<T> left, Vector256Mask<T> right)
    {
        ...
        uint leftValue = left._value & ((1U << Count) - 1);
        uint rightValue = right._value & ((1U << Count) - 1);
    ...
    [Intrinsic]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Vector256Mask<T> operator <<(Vector256Mask<T> mask, int shiftAmount)
    {
        // We only want to include bits relevant to the type
        uint value = mask._value & ((1U << Count) - 1);

        uint result = value << shiftAmount;
        return Unsafe.BitCast<uint, Vector256Mask<T>>(result);
    }
    ...
    [Intrinsic]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Vector256Mask<T> operator ~(Vector256Mask<T> mask)
    {
        ThrowHelper.ThrowForUnsupportedIntrinsicsVector256BaseType<T>();
        uint result = ~mask._value;
        return Unsafe.BitCast<uint, Vector256Mask<T>>(result);
    }
    ...
    /// <returns><c>true</c> if <paramref name="obj" /> is a <see cref="Vector256Mask{T}" /> and is equal to the current instance; otherwise, <c>false</c>.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public override bool Equals([NotNullWhen(true)] object? obj) => (obj is Vector256Mask<T> other) && Equals(other);

    /// <summary>Determines whether the specified <see cref="Vector256Mask{T}" /> is equal to the current instance.</summary>
    /// <param name="other">The <see cref="Vector256Mask{T}" /> to compare with the current instance.</param>
    /// <returns><c>true</c> if <paramref name="other" /> is equal to the current instance; otherwise, <c>false</c>.</returns>
    /// <exception cref="NotSupportedException">The type of the vector mask (<typeparamref name="T" />) is not supported.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public bool Equals(Vector256Mask<T> other) => this == other;
