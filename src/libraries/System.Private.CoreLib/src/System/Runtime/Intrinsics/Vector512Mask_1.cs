...
[DebuggerTypeProxy(typeof(Vector512MaskDebugView<>))]
[StructLayout(LayoutKind.Sequential, Size = Vector512Mask.Size)]
public readonly unsafe struct Vector512Mask<T> : IEquatable<Vector512Mask<T>>
    where T : struct
{
    // This field allows the debug view to work https://github.com/dotnet/runtime/issues/9495)
    internal readonly ulong _value;

    /// <summary>Gets a new <see cref="Vector512Mask{T}" /> that includes all elements of a vector.</summary>
    /// <exception cref="NotSupportedException">The type of the vector mask (<typeparamref name="T" />) is not supported.</exception>
    public static Vector512Mask<T> AllBitsSet
    {
        [Intrinsic]
        ...
        get
        {
            ...
            return Unsafe.BitCast<ulong, Vector512Mask<T>>(ulong.MaxValue);
        }
    }

    /// <summary>Gets the number of <typeparamref name="T" /> that are in a <see cref="Vector512Mask{T}" />.</summary>
    /// <exception cref="NotSupportedException">The type of the vector mask (<typeparamref name="T" />) is not supported.</exception>
...
#pragma warning disable 8500 // sizeof of managed types
            return Vector512Mask.Size / sizeof(T);
#pragma warning restore 8500
    ...
    /// <summary>Gets a new <see cref="Vector512Mask{T}" /> that includes no elements of a vector.</summary>
    /// <exception cref="NotSupportedException">The type of the vector mask (<typeparamref name="T" />) is not supported.</exception>
    public static Vector512Mask<T> Zero
    {
        [Intrinsic]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        get
    ...
    public static bool operator ==(Vector512Mask<T> left, Vector512Mask<T> right)
    {
        ...
        ulong leftValue = left._value & ((1UL << Count) - 1);
        ulong rightValue = right._value & ((1UL << Count) - 1);
    ...
    [Intrinsic]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Vector512Mask<T> operator <<(Vector512Mask<T> mask, int shiftAmount)
    {
        // We only want to include bits relevant to the type
        ulong value = mask._value & ((1UL << Count) - 1);

        ulong result = value << shiftAmount;
        return Unsafe.BitCast<ulong, Vector512Mask<T>>(result);
    }
    ...
    [Intrinsic]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Vector512Mask<T> operator ~(Vector512Mask<T> mask)
    {
        ThrowHelper.ThrowForUnsupportedIntrinsicsVector512BaseType<T>();
        ulong result = ~mask._value;
        return Unsafe.BitCast<ulong, Vector512Mask<T>>(result);
    }
    ...
    /// <returns><c>true</c> if <paramref name="obj" /> is a <see cref="Vector512Mask{T}" /> and is equal to the current instance; otherwise, <c>false</c>.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public override bool Equals([NotNullWhen(true)] object? obj) => (obj is Vector512Mask<T> other) && Equals(other);

    /// <summary>Determines whether the specified <see cref="Vector512Mask{T}" /> is equal to the current instance.</summary>
    /// <param name="other">The <see cref="Vector512Mask{T}" /> to compare with the current instance.</param>
    /// <returns><c>true</c> if <paramref name="other" /> is equal to the current instance; otherwise, <c>false</c>.</returns>
    /// <exception cref="NotSupportedException">The type of the vector mask (<typeparamref name="T" />) is not supported.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public bool Equals(Vector512Mask<T> other) => this == other;
    ...
    {
        // We only want to include bits relevant to the type
        ulong value = _value & ((1UL << Count) - 1);
