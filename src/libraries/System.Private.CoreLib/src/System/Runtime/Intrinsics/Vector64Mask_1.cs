...
[DebuggerTypeProxy(typeof(Vector64MaskDebugView<>))]
[StructLayout(LayoutKind.Sequential, Size = Vector64Mask.Size)]
public readonly unsafe struct Vector64Mask<T> : IEquatable<Vector64Mask<T>>
    where T : struct
{
    // This field allows the debug view to work https://github.com/dotnet/runtime/issues/9495)
    internal readonly byte _value;

    /// <summary>Gets a new <see cref="Vector64Mask{T}" /> that includes all elements of a vector.</summary>
    /// <exception cref="NotSupportedException">The type of the vector mask (<typeparamref name="T" />) is not supported.</exception>
    public static Vector64Mask<T> AllBitsSet
    {
        [Intrinsic]
        ...
        get
        {
            ...
            return Unsafe.BitCast<byte, Vector64Mask<T>>(byte.MaxValue);
        }
    }

    /// <summary>Gets the number of <typeparamref name="T" /> that are in a <see cref="Vector64Mask{T}" />.</summary>
    /// <exception cref="NotSupportedException">The type of the vector mask (<typeparamref name="T" />) is not supported.</exception>
...
#pragma warning disable 8500 // sizeof of managed types
            return Vector64Mask.Size / sizeof(T);
#pragma warning restore 8500
    ...
    /// <summary>Gets a new <see cref="Vector64Mask{T}" /> that includes no elements of a vector.</summary>
    /// <exception cref="NotSupportedException">The type of the vector mask (<typeparamref name="T" />) is not supported.</exception>
    public static Vector64Mask<T> Zero
    {
        [Intrinsic]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        get
    ...
    public static bool operator ==(Vector64Mask<T> left, Vector64Mask<T> right)
    {
        ...
        byte leftValue = (byte)(left._value & ((1 << Count) - 1));
        byte rightValue = (byte)(right._value & ((1 << Count) - 1));
    ...
    [Intrinsic]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Vector64Mask<T> operator <<(Vector64Mask<T> mask, int shiftAmount)
    {
        // We only want to include bits relevant to the type
        byte value = (byte)(mask._value & ((1 << Count) - 1));

        byte result = (byte)(value << shiftAmount);
        return Unsafe.BitCast<byte, Vector64Mask<T>>(result);
    }
    ...
    [Intrinsic]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Vector64Mask<T> operator ~(Vector64Mask<T> mask)
    {
        ThrowHelper.ThrowForUnsupportedIntrinsicsVector64BaseType<T>();
        byte result = (byte)(~mask._value);
        return Unsafe.BitCast<byte, Vector64Mask<T>>(result);
    }
    ...
    /// <returns><c>true</c> if <paramref name="obj" /> is a <see cref="Vector64Mask{T}" /> and is equal to the current instance; otherwise, <c>false</c>.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public override bool Equals([NotNullWhen(true)] object? obj) => (obj is Vector64Mask<T> other) && Equals(other);

    /// <summary>Determines whether the specified <see cref="Vector64Mask{T}" /> is equal to the current instance.</summary>
    /// <param name="other">The <see cref="Vector64Mask{T}" /> to compare with the current instance.</param>
    /// <returns><c>true</c> if <paramref name="other" /> is equal to the current instance; otherwise, <c>false</c>.</returns>
    /// <exception cref="NotSupportedException">The type of the vector mask (<typeparamref name="T" />) is not supported.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public bool Equals(Vector64Mask<T> other) => this == other;
    ...
    {
        // We only want to include bits relevant to the type
        byte value = (byte)(_value & ((1 << Count) - 1));
