    ...
    public sealed partial class TimeZoneInfo
    {
        ...
        public sealed class AdjustmentRule : IEquatable<AdjustmentRule?>, ISerializable, IDeserializationCallback
        {
                ...
                bool noDaylightTransitions)
            {
                ...
                {
                    throw new ArgumentException(SR.GetResourceString("Argument_DateTimeKindMustBeUnspecifiedOrUtc"), nameof(dateStart));
                }

                if (dateEnd.Kind != DateTimeKind.Unspecified && dateEnd.Kind != DateTimeKind.Utc)
                {
                    throw new ArgumentException(SR.GetResourceString("Argument_DateTimeKindMustBeUnspecifiedOrUtc"), nameof(dateEnd));
                }

                if (daylightTransitionStart.Equals(daylightTransitionEnd) && !noDaylightTransitions)
                {
                    throw new ArgumentException(SR.GetResourceString("Argument_TransitionTimesAreIdentical"), nameof(daylightTransitionEnd));
                }

                if (dateStart > dateEnd)
                {
                    throw new ArgumentException(SR.GetResourceString("Argument_OutOfOrderDateTimes"), nameof(dateStart));
                }
                ...
                {
                    throw new ArgumentOutOfRangeException(nameof(daylightDelta), daylightDelta, SR.GetResourceString("ArgumentOutOfRange_UtcOffset"));
                }

                if (daylightDelta.Ticks % TimeSpan.TicksPerMinute != 0)
                {
                    throw new ArgumentException(SR.GetResourceString("Argument_TimeSpanHasSeconds"), nameof(daylightDelta));
                }

                if (dateStart != DateTime.MinValue && dateStart.Kind == DateTimeKind.Unspecified && dateStart.TimeOfDay != TimeSpan.Zero)
                {
                    throw new ArgumentException(SR.GetResourceString("Argument_DateTimeHasTimeOfDay"), nameof(dateStart));
                }

                if (dateEnd != DateTime.MaxValue && dateEnd.Kind == DateTimeKind.Unspecified && dateEnd.TimeOfDay != TimeSpan.Zero)
                {
                    throw new ArgumentException(SR.GetResourceString("Argument_DateTimeHasTimeOfDay"), nameof(dateEnd));
                }
            }
