    ...
    internal static partial class BigIntegerCalculator
    {
        ...
        private readonly ref struct FastReducer
        {
            ...
            public FastReducer(ReadOnlySpan<nuint> modulus, Span<nuint> r, Span<nuint> mu, Span<nuint> q1, Span<nuint> q2)
            {
                ...
                {
                    Divide<UInt128>(r, modulus, mu);
                }
                else
                {
                    Divide<ulong>(r, modulus, mu);
                }
                _modulus = modulus;
            ...
            public int Reduce(Span<nuint> value)
            {
                ...
                // Let q2 = q1/2^(k+1) * m
                _q2.Clear();
                int l2 = DivMul(_q1[..l1], _modulus, _q2, _modulus.Length + 1);

                // Let v = (v - q2) % 2^(k+1) - i*m
                int length = SubMod(value, _q2[..l2], _modulus, _modulus.Length + 1);
                value = value[length..];
                value.Clear();
            ...
            private static int DivMul(ReadOnlySpan<nuint> left, ReadOnlySpan<nuint> right, Span<nuint> bits, int k)
            {
                ...
                // preceding division by 2^(BitsPerElement*k). To spare memory allocations
                ...
                if (left.Length > k)
                {
                    ...
                    return ActualLength(bits[..(left.Length + right.Length)]);
                }
            ...
            private static int SubMod(Span<nuint> left, ReadOnlySpan<nuint> right, ReadOnlySpan<nuint> modulus, int k)
            {
                ...
                // preceding reduction by 2^(BitsPerElement*k). Furthermore, if left is
                ...
                if (Environment.Is64BitProcess)
                {
                    SubtractSelf<Int128>(left, right);
                }
                else
                {
                    SubtractSelf<long>(left, right);
                }
                left = left[..ActualLength(left)];

                while (Compare(left, modulus) >= 0)
                {
                    if (Environment.Is64BitProcess)
                    {
                        SubtractSelf<Int128>(left, modulus);
                    }
                    else
                    {
                        SubtractSelf<long>(left, modulus);
                    }
                    left = left[..ActualLength(left)];
                }
