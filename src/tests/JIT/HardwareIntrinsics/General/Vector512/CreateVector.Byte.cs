...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorCreate__CreateVectorByte
    {
        ...
        public void RunBasicScenario()
        {
            ...
            Vector256<Byte> lower = Vector256.Create(lowerValue);

            Byte upperValue = TestLibrary.Generator.GetByte();
            Vector256<Byte> upper = Vector256.Create(upperValue);

            Vector512<Byte> result = Vector512.Create(lower, upper);
        ...
        public void RunReflectionScenario()
        {
            ...
            object result = typeof(Vector512)
                                .GetMethod(nameof(Vector512.Create), new Type[] { typeof(Vector256<Byte>), typeof(Vector256<Byte>) })
                                .Invoke(null, new object[] { lower, upper });

            ValidateResult((Vector512<Byte>)(result), lowerValue, upperValue);
        }

        private void ValidateResult(Vector512<Byte> result, Byte expectedLowerValue, Byte expectedUpperValue, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateResult(Byte[] resultElements, Byte expectedLowerValue, Byte expectedUpperValue, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512.Create(Byte): {method} failed:");
