...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorCreate__CreateVectorUInt16
    {
        ...
        public void RunBasicScenario()
        {
            ...
            Vector256<UInt16> lower = Vector256.Create(lowerValue);

            UInt16 upperValue = TestLibrary.Generator.GetUInt16();
            Vector256<UInt16> upper = Vector256.Create(upperValue);

            Vector512<UInt16> result = Vector512.Create(lower, upper);
        ...
        public void RunReflectionScenario()
        {
            ...
            object result = typeof(Vector512)
                                .GetMethod(nameof(Vector512.Create), new Type[] { typeof(Vector256<UInt16>), typeof(Vector256<UInt16>) })
                                .Invoke(null, new object[] { lower, upper });

            ValidateResult((Vector512<UInt16>)(result), lowerValue, upperValue);
        }

        private void ValidateResult(Vector512<UInt16> result, UInt16 expectedLowerValue, UInt16 expectedUpperValue, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateResult(UInt16[] resultElements, UInt16 expectedLowerValue, UInt16 expectedUpperValue, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512.Create(UInt16): {method} failed:");
