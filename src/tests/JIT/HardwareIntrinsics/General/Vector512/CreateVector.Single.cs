...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorCreate__CreateVectorSingle
    {
        ...
        public void RunBasicScenario()
        {
            ...
            Vector256<Single> lower = Vector256.Create(lowerValue);

            Single upperValue = TestLibrary.Generator.GetSingle();
            Vector256<Single> upper = Vector256.Create(upperValue);

            Vector512<Single> result = Vector512.Create(lower, upper);
        ...
        public void RunReflectionScenario()
        {
            ...
            object result = typeof(Vector512)
                                .GetMethod(nameof(Vector512.Create), new Type[] { typeof(Vector256<Single>), typeof(Vector256<Single>) })
                                .Invoke(null, new object[] { lower, upper });

            ValidateResult((Vector512<Single>)(result), lowerValue, upperValue);
        }

        private void ValidateResult(Vector512<Single> result, Single expectedLowerValue, Single expectedUpperValue, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateResult(Single[] resultElements, Single expectedLowerValue, Single expectedUpperValue, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512.Create(Single): {method} failed:");
