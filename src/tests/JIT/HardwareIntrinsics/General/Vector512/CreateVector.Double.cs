...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorCreate__CreateVectorDouble
    {
        ...
        public void RunBasicScenario()
        {
            ...
            Vector256<Double> lower = Vector256.Create(lowerValue);

            Double upperValue = TestLibrary.Generator.GetDouble();
            Vector256<Double> upper = Vector256.Create(upperValue);

            Vector512<Double> result = Vector512.Create(lower, upper);
        ...
        public void RunReflectionScenario()
        {
            ...
            object result = typeof(Vector512)
                                .GetMethod(nameof(Vector512.Create), new Type[] { typeof(Vector256<Double>), typeof(Vector256<Double>) })
                                .Invoke(null, new object[] { lower, upper });

            ValidateResult((Vector512<Double>)(result), lowerValue, upperValue);
        }

        private void ValidateResult(Vector512<Double> result, Double expectedLowerValue, Double expectedUpperValue, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateResult(Double[] resultElements, Double expectedLowerValue, Double expectedUpperValue, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512.Create(Double): {method} failed:");
