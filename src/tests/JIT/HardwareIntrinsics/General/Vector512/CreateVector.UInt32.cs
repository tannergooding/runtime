...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorCreate__CreateVectorUInt32
    {
        ...
        public void RunBasicScenario()
        {
            ...
            Vector256<UInt32> lower = Vector256.Create(lowerValue);

            UInt32 upperValue = TestLibrary.Generator.GetUInt32();
            Vector256<UInt32> upper = Vector256.Create(upperValue);

            Vector512<UInt32> result = Vector512.Create(lower, upper);
        ...
        public void RunReflectionScenario()
        {
            ...
            object result = typeof(Vector512)
                                .GetMethod(nameof(Vector512.Create), new Type[] { typeof(Vector256<UInt32>), typeof(Vector256<UInt32>) })
                                .Invoke(null, new object[] { lower, upper });

            ValidateResult((Vector512<UInt32>)(result), lowerValue, upperValue);
        }

        private void ValidateResult(Vector512<UInt32> result, UInt32 expectedLowerValue, UInt32 expectedUpperValue, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateResult(UInt32[] resultElements, UInt32 expectedLowerValue, UInt32 expectedUpperValue, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512.Create(UInt32): {method} failed:");
