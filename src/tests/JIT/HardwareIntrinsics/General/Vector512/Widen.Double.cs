...
namespace JIT.HardwareIntrinsics.General
{
    ...
    {
        private static void WidenDouble()
        {
    ...
    public sealed unsafe class VectorWidenTest__WidenDouble
    {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            Unsafe.Write(_dataTable.outLowerArrayPtr, (((Vector512<Double> Lower, Vector512<Double> Upper))(result)).Lower);
            Unsafe.Write(_dataTable.outUpperArrayPtr, (((Vector512<Double> Lower, Vector512<Double> Upper))(result)).Upper);
            ValidateResult(_dataTable.inArray1Ptr, _dataTable.outLowerArrayPtr, _dataTable.outUpperArrayPtr);
        }
        ...
        private void ValidateResult(Vector512<Single> op1, void* lowerResult, void* upperResult, [CallerMemberName] string method = "")
        {
            ...
            Unsafe.CopyBlockUnaligned(ref Unsafe.As<Double, byte>(ref outLowerArray[0]), ref Unsafe.AsRef<byte>(lowerResult), (uint)Unsafe.SizeOf<Vector512<Double>>());
            Unsafe.CopyBlockUnaligned(ref Unsafe.As<Double, byte>(ref outUpperArray[0]), ref Unsafe.AsRef<byte>(upperResult), (uint)Unsafe.SizeOf<Vector512<Double>>());
        ...
        private void ValidateResult(Single[] firstOp, Double[] lowerResult, Double[] upperResult, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector512)}.{nameof(Vector512.Widen)}<Double>(Vector512<Single>): {method} failed:");
