...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorCreate__CreateVectorUInt64
    {
        ...
        public void RunBasicScenario()
        {
            ...
            Vector256<UInt64> lower = Vector256.Create(lowerValue);

            UInt64 upperValue = TestLibrary.Generator.GetUInt64();
            Vector256<UInt64> upper = Vector256.Create(upperValue);

            Vector512<UInt64> result = Vector512.Create(lower, upper);
        ...
        public void RunReflectionScenario()
        {
            ...
            object result = typeof(Vector512)
                                .GetMethod(nameof(Vector512.Create), new Type[] { typeof(Vector256<UInt64>), typeof(Vector256<UInt64>) })
                                .Invoke(null, new object[] { lower, upper });

            ValidateResult((Vector512<UInt64>)(result), lowerValue, upperValue);
        }

        private void ValidateResult(Vector512<UInt64> result, UInt64 expectedLowerValue, UInt64 expectedUpperValue, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateResult(UInt64[] resultElements, UInt64 expectedLowerValue, UInt64 expectedUpperValue, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512.Create(UInt64): {method} failed:");
