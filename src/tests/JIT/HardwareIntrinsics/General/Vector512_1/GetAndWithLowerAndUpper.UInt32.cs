...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorGetAndWithLowerAndUpper__GetAndWithLowerAndUpperUInt32
    {
        ...
        public void RunBasicScenario()
        {
            ...
            Vector512<UInt32> result = value.WithLower(upperResult);
        ...
        public void RunReflectionScenario()
        {
            ...
            object upperResult = typeof(Vector512)
                                    ...
                                    .MakeGenericMethod(typeof(UInt32))
                                    .Invoke(null, new object[] { value });
            ValidateGetResult((Vector256<UInt32>)(lowerResult), (Vector256<UInt32>)(upperResult), values);
            ...
            ValidateWithResult((Vector512<UInt32>)(result), values);
        }

        private void ValidateGetResult(Vector256<UInt32> lowerResult, Vector256<UInt32> upperResult, UInt32[] values, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateGetResult(UInt32[] lowerResult, UInt32[] upperResult, UInt32[] values, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt32>.GetLower(): {method} failed:");
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt32>.GetUpper(): {method} failed:");
                TestLibrary.TestFramework.LogInformation($"   value: ({string.Join(", ", values)})");
        ...
        private void ValidateWithResult(Vector512<UInt32> result, UInt32[] values, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateWithResult(UInt32[] result, UInt32[] values, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt32.WithLower(): {method} failed:");
                TestLibrary.TestFramework.LogInformation($"   value: ({string.Join(", ", values)})");
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt32.WithUpper(): {method} failed:");
                TestLibrary.TestFramework.LogInformation($"   value: ({string.Join(", ", values)})");
                TestLibrary.TestFramework.LogInformation($"  result: ({string.Join(", ", result)})");
                TestLibrary.TestFramework.LogInformation(string.Empty);
