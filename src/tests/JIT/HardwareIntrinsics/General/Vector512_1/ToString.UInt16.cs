...
namespace JIT.HardwareIntrinsics.General
{
    public static partial class Program
    {
        ...
        {
            int size = Unsafe.SizeOf<Vector512<UInt16>>() / sizeof(UInt16);
            ...
            Vector512<UInt16> vector = Vector512.Create(values[0], values[1], values[2], values[3], values[4], values[5], values[6], values[7], values[8], values[9], values[10], values[11], values[12], values[13], values[14], values[15]);
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512UInt16ToString: Vector512<UInt16>.ToString() returned an unexpected result.");
