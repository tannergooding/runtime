...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorAs__AsSingle
    {
        ...
        public void RunBasicScenario()
        {
            ...
            Vector512<Single> value;

            value = Vector512.Create((float)TestLibrary.Generator.GetSingle());
        ...
        public void RunGenericScenario()
        {
            ...
            value = Vector512.Create((float)TestLibrary.Generator.GetSingle());
            Vector512<byte> byteResult = value.As<Single, byte>();
            ValidateResult(byteResult, value);

            value = Vector512.Create((float)TestLibrary.Generator.GetSingle());
            Vector512<double> doubleResult = value.As<Single, double>();
            ValidateResult(doubleResult, value);

            value = Vector512.Create((float)TestLibrary.Generator.GetSingle());
            Vector512<short> shortResult = value.As<Single, short>();
            ValidateResult(shortResult, value);

            value = Vector512.Create((float)TestLibrary.Generator.GetSingle());
            Vector512<int> intResult = value.As<Single, int>();
            ValidateResult(intResult, value);

            value = Vector512.Create((float)TestLibrary.Generator.GetSingle());
            Vector512<long> longResult = value.As<Single, long>();
            ValidateResult(longResult, value);

            value = Vector512.Create((float)TestLibrary.Generator.GetSingle());
            Vector512<sbyte> sbyteResult = value.As<Single, sbyte>();
            ValidateResult(sbyteResult, value);

            value = Vector512.Create((float)TestLibrary.Generator.GetSingle());
            Vector512<float> floatResult = value.As<Single, float>();
            ValidateResult(floatResult, value);

            value = Vector512.Create((float)TestLibrary.Generator.GetSingle());
            Vector512<ushort> ushortResult = value.As<Single, ushort>();
            ValidateResult(ushortResult, value);

            value = Vector512.Create((float)TestLibrary.Generator.GetSingle());
            Vector512<uint> uintResult = value.As<Single, uint>();
            ValidateResult(uintResult, value);

            value = Vector512.Create((float)TestLibrary.Generator.GetSingle());
            Vector512<ulong> ulongResult = value.As<Single, ulong>();
            ValidateResult(ulongResult, value);
        }
        ...
        private void ValidateResult<T>(Vector512<T> result, Vector512<Single> value, [CallerMemberName] string method = "")
            where T : struct
        {
        ...
        private void ValidateResult(Single[] resultElements, Single[] valueElements, Type targetType, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<Single>.As{targetType.Name}: {method} failed:");
