...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorGetAndWithLowerAndUpper__GetAndWithLowerAndUpperUInt16
    {
        ...
        public void RunBasicScenario()
        {
            ...
            Vector256<UInt16> lowerResult = value.GetLower();
            Vector256<UInt16> upperResult = value.GetUpper();
            ValidateGetResult(lowerResult, upperResult, values);

            Vector512<UInt16> result = value.WithLower(upperResult);
        ...
        public void RunReflectionScenario()
        {
            ...
            object upperResult = typeof(Vector512)
                                    ...
                                    .MakeGenericMethod(typeof(UInt16))
                                    .Invoke(null, new object[] { value });
            ValidateGetResult((Vector256<UInt16>)(lowerResult), (Vector256<UInt16>)(upperResult), values);
            ...
            ValidateWithResult((Vector512<UInt16>)(result), values);
        }

        private void ValidateGetResult(Vector256<UInt16> lowerResult, Vector256<UInt16> upperResult, UInt16[] values, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateGetResult(UInt16[] lowerResult, UInt16[] upperResult, UInt16[] values, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt16>.GetLower(): {method} failed:");
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt16>.GetUpper(): {method} failed:");
                TestLibrary.TestFramework.LogInformation($"   value: ({string.Join(", ", values)})");
        ...
        private void ValidateWithResult(Vector512<UInt16> result, UInt16[] values, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateWithResult(UInt16[] result, UInt16[] values, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt16.WithLower(): {method} failed:");
                TestLibrary.TestFramework.LogInformation($"   value: ({string.Join(", ", values)})");
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt16.WithUpper(): {method} failed:");
                TestLibrary.TestFramework.LogInformation($"   value: ({string.Join(", ", values)})");
                TestLibrary.TestFramework.LogInformation($"  result: ({string.Join(", ", result)})");
                TestLibrary.TestFramework.LogInformation(string.Empty);
