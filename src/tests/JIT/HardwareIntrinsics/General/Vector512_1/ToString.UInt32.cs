...
namespace JIT.HardwareIntrinsics.General
{
    public static partial class Program
    {
        ...
        {
            int size = Unsafe.SizeOf<Vector512<UInt32>>() / sizeof(UInt32);
            ...
            Vector512<UInt32> vector = Vector512.Create(values[0], values[1], values[2], values[3], values[4], values[5], values[6], values[7]);
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512UInt32ToString: Vector512<UInt32>.ToString() returned an unexpected result.");
