...
namespace JIT.HardwareIntrinsics.General
{
    public static partial class Program
    {
        ...
        {
            int size = Unsafe.SizeOf<Vector512<UInt64>>() / sizeof(UInt64);
            ...
            Vector512<UInt64> vector = Vector512.Create(values[0], values[1], values[2], values[3]);
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512UInt64ToString: Vector512<UInt64>.ToString() returned an unexpected result.");
