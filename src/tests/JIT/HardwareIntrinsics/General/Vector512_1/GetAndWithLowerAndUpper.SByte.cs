...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorGetAndWithLowerAndUpper__GetAndWithLowerAndUpperSByte
    {
        ...
        public void RunBasicScenario()
        {
            ...
            Vector256<SByte> lowerResult = value.GetLower();
            Vector256<SByte> upperResult = value.GetUpper();
            ValidateGetResult(lowerResult, upperResult, values);

            Vector512<SByte> result = value.WithLower(upperResult);
        ...
        public void RunReflectionScenario()
        {
            ...
            object upperResult = typeof(Vector512)
                                    ...
                                    .MakeGenericMethod(typeof(SByte))
                                    .Invoke(null, new object[] { value });
            ValidateGetResult((Vector256<SByte>)(lowerResult), (Vector256<SByte>)(upperResult), values);
            ...
            ValidateWithResult((Vector512<SByte>)(result), values);
        }

        private void ValidateGetResult(Vector256<SByte> lowerResult, Vector256<SByte> upperResult, SByte[] values, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateGetResult(SByte[] lowerResult, SByte[] upperResult, SByte[] values, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<SByte>.GetLower(): {method} failed:");
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<SByte>.GetUpper(): {method} failed:");
                TestLibrary.TestFramework.LogInformation($"   value: ({string.Join(", ", values)})");
        ...
        private void ValidateWithResult(Vector512<SByte> result, SByte[] values, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateWithResult(SByte[] result, SByte[] values, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<SByte.WithLower(): {method} failed:");
                TestLibrary.TestFramework.LogInformation($"   value: ({string.Join(", ", values)})");
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<SByte.WithUpper(): {method} failed:");
                TestLibrary.TestFramework.LogInformation($"   value: ({string.Join(", ", values)})");
                TestLibrary.TestFramework.LogInformation($"  result: ({string.Join(", ", result)})");
                TestLibrary.TestFramework.LogInformation(string.Empty);
