...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorGetAndWithElement__GetAndWithElementUInt327
    {
        ...
        private static readonly int ElementCount = Unsafe.SizeOf<Vector512<UInt32>>() / sizeof(UInt32);
        ...
        public void RunBasicScenario(int imm = 7, bool expectedOutOfRangeException = false)
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt32.GetElement({imm}): {nameof(RunBasicScenario)} failed to throw ArgumentOutOfRangeException.");
            ...
            try
            {
                Vector512<UInt32> result2 = value.WithElement(imm, insertedValue);
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt32.WithElement({imm}): {nameof(RunBasicScenario)} failed to throw ArgumentOutOfRangeException.");
                TestLibrary.TestFramework.LogInformation(string.Empty);
        ...
        public void RunReflectionScenario(int imm = 7, bool expectedOutOfRangeException = false)
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt32.GetElement({imm}): {nameof(RunReflectionScenario)} failed to throw ArgumentOutOfRangeException.");
                TestLibrary.TestFramework.LogInformation(string.Empty);
            ...
            try
            {
                ...
                ValidateWithResult((Vector512<UInt32>)(result2), values, insertedValue);
            }
            catch (TargetInvocationException e)
            {
                succeeded = expectedOutOfRangeException
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt32.WithElement({imm}): {nameof(RunReflectionScenario)} failed to throw ArgumentOutOfRangeException.");
                TestLibrary.TestFramework.LogInformation(string.Empty);
        ...
        private void ValidateGetResult(UInt32 result, UInt32[] values, [CallerMemberName] string method = "")
        {
            if (result != values[7])
            {
                ...
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt32.GetElement(7): {method} failed:");
        ...
        private void ValidateWithResult(Vector512<UInt32> result, UInt32[] values, UInt32 insertedValue, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateWithResult(UInt32[] result, UInt32[] values, UInt32 insertedValue, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt32.WithElement(7): {method} failed:");
                TestLibrary.TestFramework.LogInformation($"   value: ({string.Join(", ", values)})");
