...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorGetAndWithLowerAndUpper__GetAndWithLowerAndUpperSingle
    {
        ...
        public void RunBasicScenario()
        {
            ...
            Vector256<Single> lowerResult = value.GetLower();
            Vector256<Single> upperResult = value.GetUpper();
            ValidateGetResult(lowerResult, upperResult, values);

            Vector512<Single> result = value.WithLower(upperResult);
        ...
        public void RunReflectionScenario()
        {
            ...
            object upperResult = typeof(Vector512)
                                    ...
                                    .MakeGenericMethod(typeof(Single))
                                    .Invoke(null, new object[] { value });
            ValidateGetResult((Vector256<Single>)(lowerResult), (Vector256<Single>)(upperResult), values);
            ...
            ValidateWithResult((Vector512<Single>)(result), values);
        }

        private void ValidateGetResult(Vector256<Single> lowerResult, Vector256<Single> upperResult, Single[] values, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateGetResult(Single[] lowerResult, Single[] upperResult, Single[] values, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<Single>.GetLower(): {method} failed:");
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<Single>.GetUpper(): {method} failed:");
                TestLibrary.TestFramework.LogInformation($"   value: ({string.Join(", ", values)})");
        ...
        private void ValidateWithResult(Vector512<Single> result, Single[] values, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateWithResult(Single[] result, Single[] values, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<Single.WithLower(): {method} failed:");
                TestLibrary.TestFramework.LogInformation($"   value: ({string.Join(", ", values)})");
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<Single.WithUpper(): {method} failed:");
                TestLibrary.TestFramework.LogInformation($"   value: ({string.Join(", ", values)})");
                TestLibrary.TestFramework.LogInformation($"  result: ({string.Join(", ", result)})");
                TestLibrary.TestFramework.LogInformation(string.Empty);
