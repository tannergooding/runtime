...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorAs__AsUInt16
    {
        ...
        public void RunBasicScenario()
        {
            ...
            Vector512<UInt16> value;

            value = Vector512.Create((ushort)TestLibrary.Generator.GetUInt16());
        ...
        public void RunGenericScenario()
        {
            ...
            value = Vector512.Create((ushort)TestLibrary.Generator.GetUInt16());
            Vector512<byte> byteResult = value.As<UInt16, byte>();
            ValidateResult(byteResult, value);

            value = Vector512.Create((ushort)TestLibrary.Generator.GetUInt16());
            Vector512<double> doubleResult = value.As<UInt16, double>();
            ValidateResult(doubleResult, value);

            value = Vector512.Create((ushort)TestLibrary.Generator.GetUInt16());
            Vector512<short> shortResult = value.As<UInt16, short>();
            ValidateResult(shortResult, value);

            value = Vector512.Create((ushort)TestLibrary.Generator.GetUInt16());
            Vector512<int> intResult = value.As<UInt16, int>();
            ValidateResult(intResult, value);

            value = Vector512.Create((ushort)TestLibrary.Generator.GetUInt16());
            Vector512<long> longResult = value.As<UInt16, long>();
            ValidateResult(longResult, value);

            value = Vector512.Create((ushort)TestLibrary.Generator.GetUInt16());
            Vector512<sbyte> sbyteResult = value.As<UInt16, sbyte>();
            ValidateResult(sbyteResult, value);

            value = Vector512.Create((ushort)TestLibrary.Generator.GetUInt16());
            Vector512<float> floatResult = value.As<UInt16, float>();
            ValidateResult(floatResult, value);

            value = Vector512.Create((ushort)TestLibrary.Generator.GetUInt16());
            Vector512<ushort> ushortResult = value.As<UInt16, ushort>();
            ValidateResult(ushortResult, value);

            value = Vector512.Create((ushort)TestLibrary.Generator.GetUInt16());
            Vector512<uint> uintResult = value.As<UInt16, uint>();
            ValidateResult(uintResult, value);

            value = Vector512.Create((ushort)TestLibrary.Generator.GetUInt16());
            Vector512<ulong> ulongResult = value.As<UInt16, ulong>();
            ValidateResult(ulongResult, value);
        }
        ...
        private void ValidateResult<T>(Vector512<T> result, Vector512<UInt16> value, [CallerMemberName] string method = "")
            where T : struct
        {
        ...
        private void ValidateResult(UInt16[] resultElements, UInt16[] valueElements, Type targetType, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt16>.As{targetType.Name}: {method} failed:");
