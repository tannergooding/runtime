...
namespace JIT.HardwareIntrinsics.General
{
    public static partial class Program
    {
        ...
        {
            int size = Unsafe.SizeOf<Vector512<Single>>() / sizeof(Single);
            ...
            Vector512<Single> vector = Vector512.Create(values[0], values[1], values[2], values[3], values[4], values[5], values[6], values[7]);
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512SingleToString: Vector512<Single>.ToString() returned an unexpected result.");
