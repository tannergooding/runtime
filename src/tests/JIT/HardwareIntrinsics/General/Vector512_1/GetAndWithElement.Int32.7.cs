...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorGetAndWithElement__GetAndWithElementInt327
    {
        ...
        public void RunBasicScenario(int imm = 7, bool expectedOutOfRangeException = false)
        {
            ...
            Vector512<Int32> value = Vector512.Create(values[0], values[1], values[2], values[3], values[4], values[5], values[6], values[7]);
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<Int32.GetElement({imm}): {nameof(RunBasicScenario)} failed to throw ArgumentOutOfRangeException.");
            ...
            try
            {
                Vector512<Int32> result2 = value.WithElement(imm, insertedValue);
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<Int32.WithElement({imm}): {nameof(RunBasicScenario)} failed to throw ArgumentOutOfRangeException.");
                TestLibrary.TestFramework.LogInformation(string.Empty);
        ...
        public void RunReflectionScenario(int imm = 7, bool expectedOutOfRangeException = false)
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<Int32.GetElement({imm}): {nameof(RunReflectionScenario)} failed to throw ArgumentOutOfRangeException.");
                TestLibrary.TestFramework.LogInformation(string.Empty);
            ...
            try
            {
                ...
                ValidateWithResult((Vector512<Int32>)(result2), values, insertedValue);
            }
            catch (TargetInvocationException e)
            {
                succeeded = expectedOutOfRangeException
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<Int32.WithElement({imm}): {nameof(RunReflectionScenario)} failed to throw ArgumentOutOfRangeException.");
                TestLibrary.TestFramework.LogInformation(string.Empty);
        ...
        private void ValidateGetResult(Int32 result, Int32[] values, [CallerMemberName] string method = "")
        {
            if (result != values[7])
            {
                ...
                TestLibrary.TestFramework.LogInformation($"Vector512<Int32.GetElement(7): {method} failed:");
        ...
        private void ValidateWithResult(Vector512<Int32> result, Int32[] values, Int32 insertedValue, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateWithResult(Int32[] result, Int32[] values, Int32 insertedValue, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<Int32.WithElement(7): {method} failed:");
                TestLibrary.TestFramework.LogInformation($"   value: ({string.Join(", ", values)})");
