...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorAs__AsUInt32
    {
        ...
        public void RunBasicScenario()
        {
            ...
            Vector512<UInt32> value;

            value = Vector512.Create((uint)TestLibrary.Generator.GetUInt32());
        ...
        public void RunGenericScenario()
        {
            ...
            value = Vector512.Create((uint)TestLibrary.Generator.GetUInt32());
            Vector512<byte> byteResult = value.As<UInt32, byte>();
            ValidateResult(byteResult, value);

            value = Vector512.Create((uint)TestLibrary.Generator.GetUInt32());
            Vector512<double> doubleResult = value.As<UInt32, double>();
            ValidateResult(doubleResult, value);

            value = Vector512.Create((uint)TestLibrary.Generator.GetUInt32());
            Vector512<short> shortResult = value.As<UInt32, short>();
            ValidateResult(shortResult, value);

            value = Vector512.Create((uint)TestLibrary.Generator.GetUInt32());
            Vector512<int> intResult = value.As<UInt32, int>();
            ValidateResult(intResult, value);

            value = Vector512.Create((uint)TestLibrary.Generator.GetUInt32());
            Vector512<long> longResult = value.As<UInt32, long>();
            ValidateResult(longResult, value);

            value = Vector512.Create((uint)TestLibrary.Generator.GetUInt32());
            Vector512<sbyte> sbyteResult = value.As<UInt32, sbyte>();
            ValidateResult(sbyteResult, value);

            value = Vector512.Create((uint)TestLibrary.Generator.GetUInt32());
            Vector512<float> floatResult = value.As<UInt32, float>();
            ValidateResult(floatResult, value);

            value = Vector512.Create((uint)TestLibrary.Generator.GetUInt32());
            Vector512<ushort> ushortResult = value.As<UInt32, ushort>();
            ValidateResult(ushortResult, value);

            value = Vector512.Create((uint)TestLibrary.Generator.GetUInt32());
            Vector512<uint> uintResult = value.As<UInt32, uint>();
            ValidateResult(uintResult, value);

            value = Vector512.Create((uint)TestLibrary.Generator.GetUInt32());
            Vector512<ulong> ulongResult = value.As<UInt32, ulong>();
            ValidateResult(ulongResult, value);
        }
        ...
        private void ValidateResult<T>(Vector512<T> result, Vector512<UInt32> value, [CallerMemberName] string method = "")
            where T : struct
        {
        ...
        private void ValidateResult(UInt32[] resultElements, UInt32[] valueElements, Type targetType, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt32>.As{targetType.Name}: {method} failed:");
