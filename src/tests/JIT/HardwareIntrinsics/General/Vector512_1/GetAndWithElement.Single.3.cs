...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorGetAndWithElement__GetAndWithElementSingle3
    {
        ...
        private static readonly int ElementCount = Unsafe.SizeOf<Vector512<Single>>() / sizeof(Single);
        ...
        public void RunBasicScenario(int imm = 3, bool expectedOutOfRangeException = false)
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<Single.GetElement({imm}): {nameof(RunBasicScenario)} failed to throw ArgumentOutOfRangeException.");
            ...
            try
            {
                Vector512<Single> result2 = value.WithElement(imm, insertedValue);
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<Single.WithElement({imm}): {nameof(RunBasicScenario)} failed to throw ArgumentOutOfRangeException.");
                TestLibrary.TestFramework.LogInformation(string.Empty);
        ...
        public void RunReflectionScenario(int imm = 3, bool expectedOutOfRangeException = false)
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<Single.GetElement({imm}): {nameof(RunReflectionScenario)} failed to throw ArgumentOutOfRangeException.");
                TestLibrary.TestFramework.LogInformation(string.Empty);
            ...
            try
            {
                ...
                ValidateWithResult((Vector512<Single>)(result2), values, insertedValue);
            }
            catch (TargetInvocationException e)
            {
                succeeded = expectedOutOfRangeException
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<Single.WithElement({imm}): {nameof(RunReflectionScenario)} failed to throw ArgumentOutOfRangeException.");
                TestLibrary.TestFramework.LogInformation(string.Empty);
        ...
        private void ValidateGetResult(Single result, Single[] values, [CallerMemberName] string method = "")
        {
            if (result != values[3])
            {
                ...
                TestLibrary.TestFramework.LogInformation($"Vector512<Single.GetElement(3): {method} failed:");
        ...
        private void ValidateWithResult(Vector512<Single> result, Single[] values, Single insertedValue, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateWithResult(Single[] result, Single[] values, Single insertedValue, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<Single.WithElement(3): {method} failed:");
                TestLibrary.TestFramework.LogInformation($"   value: ({string.Join(", ", values)})");
