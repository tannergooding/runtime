...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorGetAndWithElement__GetAndWithElementUInt1615
    {
        ...
        private static readonly int ElementCount = Unsafe.SizeOf<Vector512<UInt16>>() / sizeof(UInt16);
        ...
        public void RunBasicScenario(int imm = 15, bool expectedOutOfRangeException = false)
        {
            ...
            Vector512<UInt16> value = Vector512.Create(values[0], values[1], values[2], values[3], values[4], values[5], values[6], values[7], values[8], values[9], values[10], values[11], values[12], values[13], values[14], values[15]);
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt16.GetElement({imm}): {nameof(RunBasicScenario)} failed to throw ArgumentOutOfRangeException.");
            ...
            try
            {
                Vector512<UInt16> result2 = value.WithElement(imm, insertedValue);
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt16.WithElement({imm}): {nameof(RunBasicScenario)} failed to throw ArgumentOutOfRangeException.");
                TestLibrary.TestFramework.LogInformation(string.Empty);
        ...
        public void RunReflectionScenario(int imm = 15, bool expectedOutOfRangeException = false)
        {
            ...
            try
            {
                object result = typeof(Vector512)
                                    .GetMethod(nameof(Vector512.GetElement))
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt16.GetElement({imm}): {nameof(RunReflectionScenario)} failed to throw ArgumentOutOfRangeException.");
                TestLibrary.TestFramework.LogInformation(string.Empty);
            ...
            try
            {
                object result2 = typeof(Vector512)
                                    .GetMethod(nameof(Vector512.WithElement))
                                    .MakeGenericMethod(typeof(UInt16))
                                    .Invoke(null, new object[] { value, imm, insertedValue });
                ValidateWithResult((Vector512<UInt16>)(result2), values, insertedValue);
            }
            catch (TargetInvocationException e)
            {
                succeeded = expectedOutOfRangeException
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt16.WithElement({imm}): {nameof(RunReflectionScenario)} failed to throw ArgumentOutOfRangeException.");
                TestLibrary.TestFramework.LogInformation(string.Empty);
        ...
        private void ValidateGetResult(UInt16 result, UInt16[] values, [CallerMemberName] string method = "")
        {
            if (result != values[15])
            {
                ...
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt16.GetElement(15): {method} failed:");
        ...
        private void ValidateWithResult(Vector512<UInt16> result, UInt16[] values, UInt16 insertedValue, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateWithResult(UInt16[] result, UInt16[] values, UInt16 insertedValue, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt16.WithElement(15): {method} failed:");
                TestLibrary.TestFramework.LogInformation($"   value: ({string.Join(", ", values)})");
