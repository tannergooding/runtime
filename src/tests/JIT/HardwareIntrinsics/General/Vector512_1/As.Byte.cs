...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorAs__AsByte
    {
        ...
        public void RunBasicScenario()
        {
            ...
            value = Vector512.Create((byte)TestLibrary.Generator.GetByte());
        ...
        public void RunGenericScenario()
        {
            ...
            value = Vector512.Create((byte)TestLibrary.Generator.GetByte());
            Vector512<byte> byteResult = value.As<Byte, byte>();
            ValidateResult(byteResult, value);

            value = Vector512.Create((byte)TestLibrary.Generator.GetByte());
            Vector512<double> doubleResult = value.As<Byte, double>();
            ValidateResult(doubleResult, value);

            value = Vector512.Create((byte)TestLibrary.Generator.GetByte());
            Vector512<short> shortResult = value.As<Byte, short>();
            ValidateResult(shortResult, value);

            value = Vector512.Create((byte)TestLibrary.Generator.GetByte());
            Vector512<int> intResult = value.As<Byte, int>();
            ValidateResult(intResult, value);

            value = Vector512.Create((byte)TestLibrary.Generator.GetByte());
            Vector512<long> longResult = value.As<Byte, long>();
            ValidateResult(longResult, value);

            value = Vector512.Create((byte)TestLibrary.Generator.GetByte());
            Vector512<sbyte> sbyteResult = value.As<Byte, sbyte>();
            ValidateResult(sbyteResult, value);

            value = Vector512.Create((byte)TestLibrary.Generator.GetByte());
            Vector512<float> floatResult = value.As<Byte, float>();
            ValidateResult(floatResult, value);

            value = Vector512.Create((byte)TestLibrary.Generator.GetByte());
            Vector512<ushort> ushortResult = value.As<Byte, ushort>();
            ValidateResult(ushortResult, value);

            value = Vector512.Create((byte)TestLibrary.Generator.GetByte());
            Vector512<uint> uintResult = value.As<Byte, uint>();
            ValidateResult(uintResult, value);

            value = Vector512.Create((byte)TestLibrary.Generator.GetByte());
            Vector512<ulong> ulongResult = value.As<Byte, ulong>();
            ValidateResult(ulongResult, value);
        }
        ...
        private void ValidateResult<T>(Vector512<T> result, Vector512<Byte> value, [CallerMemberName] string method = "")
            where T : struct
        {
        ...
        private void ValidateResult(Byte[] resultElements, Byte[] valueElements, Type targetType, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<Byte>.As{targetType.Name}: {method} failed:");
