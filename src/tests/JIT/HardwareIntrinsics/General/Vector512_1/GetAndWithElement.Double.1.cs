...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorGetAndWithElement__GetAndWithElementDouble1
    {
        ...
        public void RunBasicScenario(int imm = 1, bool expectedOutOfRangeException = false)
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<Double.GetElement({imm}): {nameof(RunBasicScenario)} failed to throw ArgumentOutOfRangeException.");
            ...
            try
            {
                Vector512<Double> result2 = value.WithElement(imm, insertedValue);
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<Double.WithElement({imm}): {nameof(RunBasicScenario)} failed to throw ArgumentOutOfRangeException.");
                TestLibrary.TestFramework.LogInformation(string.Empty);
        ...
        public void RunReflectionScenario(int imm = 1, bool expectedOutOfRangeException = false)
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<Double.GetElement({imm}): {nameof(RunReflectionScenario)} failed to throw ArgumentOutOfRangeException.");
                TestLibrary.TestFramework.LogInformation(string.Empty);
            ...
            try
            {
                ...
                ValidateWithResult((Vector512<Double>)(result2), values, insertedValue);
            }
            catch (TargetInvocationException e)
            {
                succeeded = expectedOutOfRangeException
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<Double.WithElement({imm}): {nameof(RunReflectionScenario)} failed to throw ArgumentOutOfRangeException.");
                TestLibrary.TestFramework.LogInformation(string.Empty);
        ...
        private void ValidateGetResult(Double result, Double[] values, [CallerMemberName] string method = "")
        {
            if (result != values[1])
            {
                ...
                TestLibrary.TestFramework.LogInformation($"Vector512<Double.GetElement(1): {method} failed:");
        ...
        private void ValidateWithResult(Vector512<Double> result, Double[] values, Double insertedValue, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateWithResult(Double[] result, Double[] values, Double insertedValue, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<Double.WithElement(1): {method} failed:");
                TestLibrary.TestFramework.LogInformation($"   value: ({string.Join(", ", values)})");
