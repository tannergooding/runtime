...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorGetAndWithLowerAndUpper__GetAndWithLowerAndUpperUInt64
    {
        ...
        public void RunBasicScenario()
        {
            ...
            Vector256<UInt64> lowerResult = value.GetLower();
            Vector256<UInt64> upperResult = value.GetUpper();
            ValidateGetResult(lowerResult, upperResult, values);

            Vector512<UInt64> result = value.WithLower(upperResult);
        ...
        public void RunReflectionScenario()
        {
            ...
            object upperResult = typeof(Vector512)
                                    ...
                                    .MakeGenericMethod(typeof(UInt64))
                                    .Invoke(null, new object[] { value });
            ValidateGetResult((Vector256<UInt64>)(lowerResult), (Vector256<UInt64>)(upperResult), values);
            ...
            ValidateWithResult((Vector512<UInt64>)(result), values);
        }

        private void ValidateGetResult(Vector256<UInt64> lowerResult, Vector256<UInt64> upperResult, UInt64[] values, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateGetResult(UInt64[] lowerResult, UInt64[] upperResult, UInt64[] values, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt64>.GetLower(): {method} failed:");
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt64>.GetUpper(): {method} failed:");
                TestLibrary.TestFramework.LogInformation($"   value: ({string.Join(", ", values)})");
        ...
        private void ValidateWithResult(Vector512<UInt64> result, UInt64[] values, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateWithResult(UInt64[] result, UInt64[] values, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt64.WithLower(): {method} failed:");
                TestLibrary.TestFramework.LogInformation($"   value: ({string.Join(", ", values)})");
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<UInt64.WithUpper(): {method} failed:");
                TestLibrary.TestFramework.LogInformation($"   value: ({string.Join(", ", values)})");
                TestLibrary.TestFramework.LogInformation($"  result: ({string.Join(", ", result)})");
                TestLibrary.TestFramework.LogInformation(string.Empty);
