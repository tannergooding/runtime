...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorAs__AsSByte
    {
        ...
        public void RunBasicScenario()
        {
            ...
            value = Vector512.Create((sbyte)TestLibrary.Generator.GetSByte());
        ...
        public void RunGenericScenario()
        {
            ...
            value = Vector512.Create((sbyte)TestLibrary.Generator.GetSByte());
            Vector512<byte> byteResult = value.As<SByte, byte>();
            ValidateResult(byteResult, value);

            value = Vector512.Create((sbyte)TestLibrary.Generator.GetSByte());
            Vector512<double> doubleResult = value.As<SByte, double>();
            ValidateResult(doubleResult, value);

            value = Vector512.Create((sbyte)TestLibrary.Generator.GetSByte());
            Vector512<short> shortResult = value.As<SByte, short>();
            ValidateResult(shortResult, value);

            value = Vector512.Create((sbyte)TestLibrary.Generator.GetSByte());
            Vector512<int> intResult = value.As<SByte, int>();
            ValidateResult(intResult, value);

            value = Vector512.Create((sbyte)TestLibrary.Generator.GetSByte());
            Vector512<long> longResult = value.As<SByte, long>();
            ValidateResult(longResult, value);

            value = Vector512.Create((sbyte)TestLibrary.Generator.GetSByte());
            Vector512<sbyte> sbyteResult = value.As<SByte, sbyte>();
            ValidateResult(sbyteResult, value);

            value = Vector512.Create((sbyte)TestLibrary.Generator.GetSByte());
            Vector512<float> floatResult = value.As<SByte, float>();
            ValidateResult(floatResult, value);

            value = Vector512.Create((sbyte)TestLibrary.Generator.GetSByte());
            Vector512<ushort> ushortResult = value.As<SByte, ushort>();
            ValidateResult(ushortResult, value);

            value = Vector512.Create((sbyte)TestLibrary.Generator.GetSByte());
            Vector512<uint> uintResult = value.As<SByte, uint>();
            ValidateResult(uintResult, value);

            value = Vector512.Create((sbyte)TestLibrary.Generator.GetSByte());
            Vector512<ulong> ulongResult = value.As<SByte, ulong>();
            ValidateResult(ulongResult, value);
        }
        ...
        private void ValidateResult<T>(Vector512<T> result, Vector512<SByte> value, [CallerMemberName] string method = "")
            where T : struct
        {
        ...
        private void ValidateResult(SByte[] resultElements, SByte[] valueElements, Type targetType, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"Vector512<SByte>.As{targetType.Name}: {method} failed:");
